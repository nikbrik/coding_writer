...
Обычный интерактивный `cw` открывает TUI: пользователь видит задачу, план, planning swarm, переходы, файлы, evidence, warnings и memory proposal в одном рабочем интерфейсе. `cw --once --input <text>` остаётся one-shot режимом для скриптов и проверок.
