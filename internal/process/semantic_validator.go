...
func (v *SemanticValidator) ResolveIntent(ctx context.Context, input SemanticIntentInput) (SemanticIntentResult, error) {
	...
	res, err := v.complete(ctx, semanticIntentSystemPrompt(), payload)
	if err != nil {
		return SemanticIntentResult{}, err
	}
	var parsed struct {
	...
	if err := decodeSemanticJSON(res, &parsed); err != nil {
		return SemanticIntentResult{}, err
	}
...
func (v *SemanticValidator) ValidateResponse(ctx context.Context, input SemanticValidationInput) ([]string, error) {
	...
	res, err := v.complete(ctx, semanticValidationSystemPrompt(), payload)
	if err != nil {
		return nil, err
	}
	var parsed struct {
...
Classify the user's intent from meaning, not exact words. Prefer the deterministic action when it is reasonable. Never invent task state.`
}
