...
- Day 11/12/13/14 покрыты acceptance tests: `tests/day_acceptance_test.go` и process-control tests in `tests/process_acceptance_test.go`; Day 15 имеет live chat-driven manual proof in `docs/manual-testing-day15.md`, а `scripts/manual-day15-user-flow.sh` остаётся deterministic regression smoke.
...
15. Для task-scoped работы пользователь формулирует цель в chat, подтверждает план и просит проверить/завершить обычной фразой; приложение само создаёт task, запускает allowlisted verification из approved plan/criteria, применяет lifecycle gates и доводит задачу до `done` только после принятой validation.
...
- когда пользователь пишет `проверь`, `проверь и заверши` или аналогичную фразу, приложение само выбирает allowlisted verification из approved plan/criteria;
...
- если безопасная проверка не выводится из approved plan/criteria, ассистент уточняет acceptance criterion в chat или блокирует `done`, но не перекладывает управление state machine на пользователя;
...
- основной Day 15 flow идёт через `assistant chat`, а не через `/task move`, `/task step`, `/task expect`, direct storage edits или JSON edits;
...
- trusted verification evidence для acceptance path должно быть выдано приложением автоматически после semantic intent signal из strict JSON referee, взято из approved plan/criteria, сохранено как evidence record и использоваться lifecycle gate;
...
- если criteria требуют tests/verification, приложение само выводит allowlisted verification command из approved plan/criteria и сохраняет evidence;
...
- запустить user-facing flow через `assistant chat` или `assistant chat --once --input <text>` без `--json`;
...
- убедиться, что приложение само выполнило allowlisted verification из approved plan/criteria;
...
- user-facing Day 15 P0 path использует natural chat; если acceptance criteria требуют test/verification evidence, приложение запускает allowlisted verification из approved plan/criteria только после semantic intent signal;
...
- automatic trusted verification from approved plan/criteria after semantic `ready_for_validation`/`ready_for_done` intent signal;
