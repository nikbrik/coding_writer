...
- Day 11/12/13 покрыты acceptance tests: `tests/day_acceptance_test.go` и process-control tests in `tests/process_acceptance_test.go`.
- Process loop уже реализован через `ProcessController`, `StagePolicyRegistry`, `StagePromptFactory`, validators, bounded retry, `TransitionGate`, semantic invariant validator и `process_audit.jsonl`.
...
assistant task start <title>
assistant task status --json
assistant task move <stage>
assistant task step <text>
assistant task expect <action>
...
- scriptable P0 включает `assistant chat --once --input <text> --json`, `assistant chat --once --render-prompt --input <text> --json`, `assistant memory list|propose|apply|proposals`, `assistant profiles list|show|set|create`, `assistant task start|status|move|step|expect|pause|resume`, `assistant process audit`, `assistant privacy`;
- `/task plan` и `/task criteria` реализованы как REPL-only conveniences для plan/acceptance criteria;
- `/task done`, `/task stage`, `/task decision` и top-level `assistant task plan|criteria|done|stage|decision` не реализованы в текущем коде.
