# PRD: AI coding agent CLI в классе Claude Code / Codex CLI

## 1. Контекст

Проект недели: перейти от обычного prompt/response-бота к минимальному stateful-ассистенту. Но конечная цель продукта шире: сделать terminal-first AI coding agent CLI в том же классе продуктов, что Claude Code и Codex CLI.

Это не generic chatbot, не memory demo и не набор debug-команд вокруг LLM. Пользовательская модель должна быть такой: разработчик открывает репозиторий, запускает ассистента в терминале, формулирует задачу обычным языком, а ассистент автономно планирует работу, понимает состояние репозитория, предлагает/применяет изменения через контролируемые tools, запускает проверки, объясняет результат и ведёт задачу до завершения через понятный chat flow.

Текущий P0/MVP — это control-plane срез такого coding agent: state, memory, profile, stage-aware prompting, validation gates, audit и app-owned verification. Отсутствие полноценных file edit/shell tools в P0 не означает, что продукт должен оставаться странной chat-утилитой; это временная граница этапа. P1 обязан двигаться к repo-aware agent workflow: read files, edit files with approvals, run commands/tests, inspect diffs, recover from failures and keep the whole interaction inside the assistant chat.

`day11.md`, `day12.md`, `day13.md`, `day14.md` и `day15.md` считаются жёсткими критериями приёмки. Лекция `03-memory-state-notes.md` задаёт архитектурные принципы: memory layers, персонализация, task state machine, инварианты, prompt builder, контролируемые переходы и сохранение состояния.
...
- Day 11/12/13/14 покрыты acceptance tests: `tests/day_acceptance_test.go` и process-control tests in `tests/process_acceptance_test.go`; Day 15 имеет live chat-driven manual proof in `docs/manual-testing-demo.md`, а `scripts/manual-day15-user-flow.sh` остаётся deterministic regression smoke.
- Process loop уже реализован через `ProcessController`, `StagePolicyRegistry`, `StagePromptFactory`, validators, bounded retry, `TransitionGate`, `LifecycleGate`, prompt improver, planning swarm, microtask agent runner, trusted evidence store, semantic invariant validator и `process_audit.jsonl`.
...
- в целевом продукте умеет читать файлы, предлагать/применять изменения, запускать команды и тесты через безопасный approval/tool layer;
...
- использует planning swarm, prompt improvement, microtask agents и trusted verification evidence как контролируемые process primitives;
...
MVP должен быть небольшим, но архитектурно правильным. Главная ценность P0 — не в количестве tools, а в фундаменте для coding-agent CLI: явное состояние, память, профиль, lifecycle, semantic validation, audit и app-owned verification. Эти компоненты нужны, чтобы следующий слой file/shell tools не превратился в небезопасную оболочку вокруг LLM.

P0 считается первым рабочим срезом Claude Code / Codex CLI-like продукта, а не финальным продуктом. Поэтому любые UX/architecture решения должны проверяться вопросом: помогает ли это разработчику решать задачи в репозитории через chat-driven autonomous workflow, или уводит продукт в сторону operator/debug utility?
...
15. Для task-scoped работы пользователь формулирует цель в chat, подтверждает план и просит проверить/завершить обычной фразой; приложение само создаёт task, получает exact verification command через `VerificationResolver`, запускает только allowlisted verification, применяет lifecycle gates и доводит задачу до `done` только после принятой validation.
