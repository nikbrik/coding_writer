...
- `tool_result` остаётся в MVP только если он реально нужен в Day 13 flow; иначе его надо удалить из всех документов.
...
- `tool_result`: нужно дождаться результата инструмента;
- `user_confirmation`: нужно подтверждение пользователя;
- `none`: задача завершена или стоит на паузе.
...
- race/locking tests for file storage;
