...
- пользователь явно пишет `/task start`, `/task plan` или подтверждает LLM proposal `[work]`;
...
## 8. LLM memory classification

После каждого значимого ответа ассистент запускает отдельный запрос к той же выбранной модели OpenRouter или к дешёвой модели, выбранной для memory tasks.
