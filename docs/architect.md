...
- хранение `status`: active, paused, done;
...
7. TaskStateManager проверяет status: active, paused или done
...
10. OpenRouterProvider отправляет request
11. Response возвращается в CLI
12. CLI печатает ответ
13. MemoryManager дописывает user/assistant сообщения в short-term history
14. MemoryClassifier получает user message + assistant response
15. MemoryClassifier вызывает OpenRouter и возвращает MemoryProposal
16. InvariantChecker проверяет proposal на секреты и layer rules
17. CLI показывает proposal пользователю
18. Пользователь подтверждает, редактирует или отклоняет
19. MemoryManager сохраняет accepted records в отдельные хранилища
```
...
17. Task state machine transition checks.
18. Task state current step and expected action.
19. `/task pause` and `/task resume` persistence.
20. Tests for day11/day12/day13 acceptance criteria.
