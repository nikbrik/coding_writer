...
Ключевое ограничение: LLM не пишет память напрямую. Она может предложить сохранить факт, но запись делает только приложение после явной команды пользователя или подтверждённого flow.
...
MVP должен retry делать только для provider errors с timeout/temporary network failure. Validation errors не retry, а возвращаются пользователю как локальный отказ.
