...
<storage_root>/
  ...
  sessions/
    <session_id>/
      ...
      prompts.jsonl        # only when ASSISTANT_PROMPT_AUDIT=1
      .last_activity
  tasks/
    current.json
    <task_id>.json
...
Current note: raw `transcript.md` is not written by the P0 implementation. `logs/` is created for extension, but there is no `app.log` writer in current code.
...
- API key must not be persisted in config, profiles, memory, prompt audit, or audit data.
...
3. `ProcessController.Preflight` checks current task state, paused/done hard gates and selected action before provider setup.
4. App загружает active profile, current task and memory bundle only after hard gates pass.
5. `ActionRouter` resolves `ActionKind`; `StagePolicyRegistry` validates the action against current stage.
6. `StagePromptFactory` supplies trusted process/stage prompts.
7. `PromptBuilder` собирает `[]ChatMessage` со stage/current_step/expected_action, active profile and memory blocks.
8. Provider payload is redacted for secret-like text.
9. `OpenRouterProvider` or `FakeProvider` sends chat completion request.
10. `ResponseParser` parses structured output when `ActionKind` requires schema.
11. Validators reject wrong stage, forbidden action, unverified claims or validation-stage implementation.
12. `RetryController` performs bounded correction retries for fixable parse/validation errors.
13. `TransitionGate` applies allowed chat-driven transitions only after validation.
14. `ProcessAuditStore` records provider calls, accepted/rejected/retried/transitioned decisions.
15. Accepted output only then goes to short-term memory and memory classifier flow.
16. `MemoryClassifier` returns proposal; `ProposalStore` saves audit; CLI shows proposal.
17. User confirms/edits/rejects; `MemoryManager` writes accepted records to separate layers.
```
...
MemoryManager
  owns: <storage_root>/sessions/<session_id>/short_term.jsonl, <storage_root>/tasks/<task_id>/work_memory.jsonl, <storage_root>/long_term/*
...
ProcessAuditStore
  owns: <storage_root>/process_audit.jsonl
  returns: accepted/rejected/retried/provider_call/transition events
```
...
- profile, memory, task state, short history, prompt audit data, and classifier output are untrusted data;
...
### 7.9. Current stateful-agent loop

Текущий controlled loop для normal chat exchange:

```text
User request
  -> deterministic ActionRouter resolves ActionKind
  ...
  -> if valid: show accepted result
  -> update short-term memory
  -> run memory classifier proposal
  -> wait for user confirmation
```

LLM уже работает внутри контролируемой стадии. Например, в `planning` она не должна делать implementation claims, а в `validation` не должна добавлять fixes/new features. Нарушения отклоняются до accepted short-memory persistence.
...
{
  "id": "senior",
  "display_name": "Senior engineer profile",
  "style": {
    "language": "ru",
    "detail": "low",
    "tone": "direct"
...
- не хранить API key в `docs/`, memory files, profiles, prompt audit;
...
- raw transcripts are not written in P0; raw prompt audit is opt-in through `ASSISTANT_RAW_PROMPT_AUDIT=1`;
...
- `TestChatOnceRenderPromptJSONUsesFakeProviderAndProfile`;
- `TestCLIP0DayFlowsUseScriptableCommands`;
...
- `TestAtomicWriteRejectsSymlinkTarget`;
- `TestProviderTimeoutAndTypedErrors`;
- `TestDuplicateProposalApplyIsIdempotent`;
- `TestProcessPausedTaskDoesNotCallProvider`;
- `TestProcessPlanningRejectsImplementationOutput`;
- `TestProcessInvalidValidationRetriesThenBlocks`;
...
Historical pre-implementation readiness is satisfied in current code. The contract now acts as a regression guard:
